SUMMARY = "Minimal VTLinux Qt5 EGLFS image for SignaSteam"
DESCRIPTION = "Minimal embedded Linux image with Qt5 EGLFS backend and SignaSteam application"
LICENSE = "MIT"
...
# The local display is owned by the boot splash and then SignaSteam - a
# getty login prompt on tty1 (same physical framebuffer) paints its
# /etc/issue banner and "login:" prompt right over whatever psplash/the
# app drew there (confirmed on-device). Serial debug access via ttyS0 is
# unaffected since it's a separate console.
